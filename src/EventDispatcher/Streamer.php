...
class Streamer implements Emitter, Listener
{
    ...
    public function __construct(History $history)
    {
        ...
        $this->readTimeout = (float) Config::get('streamer.stream_read_timeout', 1.0);
        $this->listenTimeout = (float) Config::get('streamer.listen_timeout', 1.0);
        $this->readSleep = (float) Config::get('streamer.read_sleep', 1.0);
    ...
    public function emit(Event $event, string $id = '*'): string
    {
        $meta = [
            'type' => $event->type(),
            'domain' => Config::get('streamer.domain'),
            'name' => $event->name(),
            'created' => time(),
        ];
