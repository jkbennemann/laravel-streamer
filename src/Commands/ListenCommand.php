...
class ListenCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        foreach ($events as $event) {
            ...
            $this->info("There are no local listeners associated with '$event' event in configuration.");
            $missingListeners++;
        }
    ...
    private function listen(array $events, callable $handler): void
    {
        try {
            ...
            $this->info('Starting listener again due to unexpected error.');
            if ($this->maxAttempts !== null) {
                $this->info("Attempts left: $this->maxAttempts");
                $this->maxAttempts--;
            }
